...
import time
import csv
import os  # Added to check if CSV file exists
...
# Import CSV module and initialize CSV files
csv_file = 'vehicle_counts.csv'
script_times_file = 'script_times.csv'

# Check if CSV files exist; if not, write headers
if not os.path.isfile(csv_file):
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Date', 'Hour', 'StartTime', 'EndTime', 'VehicleCount'])

if not os.path.isfile(script_times_file):
    with open(script_times_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['ScriptStartTime', 'ScriptEndTime'])

# Initialize time variables for hourly count
start_time = time.time()
last_hour = time.localtime(start_time).tm_hour
period_start_time = start_time  # Keep track of the start time of the period
...
try:
    while True:
        ...
        # Add hourly vehicle count logging
        current_time = time.time()
        current_hour = time.localtime(current_time).tm_hour

        if current_hour != last_hour:
            # The end of the counting period is now
            period_end_time = current_time
            # Write the vehicle_count to the CSV file
            date_str = time.strftime('%Y-%m-%d', time.localtime(current_time))
            # Format start and end times
            start_time_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(period_start_time))
            end_time_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(period_end_time))
            with open(csv_file, 'a', newline='') as f:
                writer = csv.writer(f)
                # Write the date, hour, start time, end time, and vehicle count
                writer.writerow([date_str, last_hour, start_time_str, end_time_str, vehicle_count])
            # Reset vehicle_count and counted_ids
            vehicle_count = 0
            counted_ids = set()
            # Update last_hour and period_start_time
    ...
    # Write the last counting period's data to the CSV file before exiting
    current_time = time.time()
    date_str = time.strftime('%Y-%m-%d', time.localtime(current_time))
    # Format start and end times
    start_time_str_period = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(period_start_time))
    end_time_str_period = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(current_time))
    with open(csv_file, 'a', newline='') as f:
        writer = csv.writer(f)
        # Write the date, hour, start time, end time, and vehicle count
        writer.writerow([date_str, last_hour, start_time_str_period, end_time_str_period, vehicle_count])

    # Record script end time
    script_end_time = time.time()
    # Format script start and end times
    start_time_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(script_start_time))
    end_time_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(script_end_time))
    # Write script start and end times to 'script_times.csv'
    with open(script_times_file, 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([start_time_str, end_time_str])
